#!/usr/bin/env python3
"""
debug_one_sample_updated.py  –  run an IEKF on a single TDCR sample
                             and visualise the *entire* backbone
                             (ground‑truth vs. IEKF estimate) **with**
                             orientation triads at the IMU positions,
                             matching the reference `shape_ekf_6d.py`.

Usage
-----
python debug_one_sample_updated.py --idx 0 \
                                   --gt  tdcr_gt_samples.npz \
                                   --meas tdcr_meas_samples.npz \
                                   --max-iter 30

Author: Guoqing Zhang (updated May‑2025)
"""
...
def run_iterated_EKF(q_meas_frame, max_iter=30, tol=1e-6):
    ...
    for _ in range(max_iter):
        ...
        innov_log.append(norm(r))

        if norm(m_new - m_est) < tol:
            m_est, P_est = m_new, P_new; break
        m_est, P_est = m_new, P_new

    return m_est, innov_log
...
def plot_frame(ax, origin, Rm, color='k', scale=5.0):
...
def main():
    ...
    p.add_argument('--idx', type=int, default=19)
    ...
    m_hat, innov_log = run_iterated_EKF(q_meas, max_iter=args.max_iter)

    # tip error ------------------------------------------------------------
    T_hat = fwd_frame(m_hat, 1.0, np.array([0,0,L_PHYS_MM]))
    p_hat = T_hat[:3,3]; p_gt = T_gt[-1,:3,3]*1000.0
    pos_err = norm(p_hat - p_gt)
    rot_err = np.degrees(np.arccos(np.clip((np.trace(T_gt[-1,:3,:3].T@T_hat[:3,:3])-1)/2, -1,1)))
    print(f"Sample #{args.idx}\npos err: {pos_err:.3f} mm | rot err: {rot_err:.3f} deg\niterations: {len(innov_log)} | final innov: {innov_log[-1]:.2e}")

    # convergence plot -----------------------------------------------------
    plt.figure(figsize=(4,3))
    plt.semilogy(innov_log, '-o')
    plt.xlabel('IEKF iteration'); plt.ylabel('innovation norm')
    plt.title(f'Convergence – sample {args.idx}')
    plt.grid(True); plt.tight_layout(); plt.show()

    # backbone & IMU frames -------------------------------------------------
    s_dense = np.linspace(0,1,160)
    pos_est = fwd_positions(m_hat, s_dense)
    pos_gt  = T_gt[:,:3,3] * 1000.0  # metres → mm

    fig = plt.figure(figsize=(6,5))
    ax = fig.add_subplot(111, projection='3d')
    ax.plot(pos_gt[:,0], pos_gt[:,1], pos_gt[:,2], 'k-', lw=2, label='Ground truth')
    ax.plot(pos_est[:,0], pos_est[:,1], pos_est[:,2], 'r--', lw=2, label='IEKF estimate')

    e3 = np.array([0,0,L_PHYS_MM])
    for s in IMU_S:
        # ground‑truth frame
        idx_gt = int(round(s*(len(T_gt)-1)))
        Tg = T_gt[idx_gt]; Rg = Tg[:3,:3]; pg = Tg[:3,3]*1000.0
    ...
    ax.set_title('Backbone shape with IMU frames')
