...
def estimate_external_wrench_from_tip_pose(tip_pose_target, tau,
                                           ...
                                           num_disks=20):
    """
    Estimate the external wrench [Fx, Fy, Fz, Mx, My, Mz] given a target tip pose and cable tensions.
    Uses least squares optimization with a bounded number of evaluations to prevent stalling.
    """
    ...
    x0 = np.array([0.0, 0.0, -0.02, 0.0, 0.001, 0.0])  # Better initial guess based on known load
...
if __name__ == '__main__':
    ...
    for nd in disks_list:
        ...
        for _ in range(N_repeat):
            ...
            model = CosseratRodModel(length=L,
                backbone_radius=r,
                youngs_modulus=E,
                num_disks=nd)
            t0 = time.perf_counter()
