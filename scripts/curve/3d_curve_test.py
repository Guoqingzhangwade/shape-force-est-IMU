...
def forward_kinematics_single(m, s, phi_func):
    ...
    e3 = np.array([0, 0, 1*total_arc_length])  # Local tangent unit vector
...
# Function to compute the tangent vector at a given s
def tangent_vector(m, s, phi_func):
    u_s = phi_func(s) @ m
    tangent = np.array([u_s[0], u_s[1], 1])
    return tangent / np.linalg.norm(tangent)

# Function to compute the real arc length as a function of s
def real_arc_length(m, phi_func):
    def integrand(s):
        tangent = tangent_vector(m, s, phi_func)
        return np.linalg.norm(tangent)

    arc_length, _ = quad(integrand, 0, 1)
    return arc_length * total_arc_length
...
m_true = np.random.uniform(-4.5, 4.5, 9)
# ipdb.set_trace()

# Compute the true transformations using m_true
s_values = np.array([0.2, 0.5, 1.0])
true_rotations, true_positions = forward_kinematics_rotations(m_true, s_values, phi_func)

# Calculate the real arc length
arc_length = real_arc_length(m_true, phi_func)
print(f"Real arc length (mm): {arc_length}")
...
def plot_3d_true_estimated_curves(m_true, m_estimated, s_values, phi_func):
    ...
    s_full = np.linspace(0, s_values[-1], 100)
