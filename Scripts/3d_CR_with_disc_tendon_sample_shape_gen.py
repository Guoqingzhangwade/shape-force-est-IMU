...
total_arc_length = 60.0  # Changed from 100 mm to 40 mm
...
def plot_3d_curve_with_disks_and_wires(ax, m_true, phi_func, k=10):
    # fig = plt.figure()
    # ax = fig.add_subplot(111, projection='3d')

    # Disk and attachment parameters
    disk_radius = 10.0  # Radius of disks
    disk_thickness = 2.0  # Thickness of disks
    attachment_radius = 7.0  # Radius for attachment points (between center and edge)
    angles = [0, 90, 180, 270]
    angles_rad = np.deg2rad(angles)

    # Compute positions and rotations at disk locations
    disk_s_values = np.linspace(0, 1, 3)  # 7 disks along the backbone
    ...
    world_origin = np.array([10, 10, 0])  # Set to (0, 0, 0)
    ...
    for w, points in enumerate(wire_points):
        ax.plot(points[:, 0], points[:, 1], points[:, 2], color='dodgerblue', alpha = 1, linestyle='-', linewidth=1, label='Wires' if w == 0 else "")
    ...
    # Remove title if any
    ax.set_title('')
    # ax.set_title(r'$\mathbf{m} = \left[\frac{\pi}{3}\mathrm{k}, -\frac{\pi}{3}\mathrm{k}, \frac{\pi}{3}\mathrm{k}\right]^\mathrm{T}$', y=1.05)
    ...
    # Do not visualize the numbers of the axis
    ax.set_xticklabels([])  
    ax.set_yticklabels([])
    ax.set_zticklabels([])
...
if __name__ == "__main__":
    ...
    # Genrate a 3 by 3 plots with 9 subplots
    fig, axs = plt.subplots(3, 3, figsize=(8, 10), constrained_layout=True)
    ...
    axs[0, 0].set_title(r'$\mathbf{m} = \left[0, 0, 0, \frac{\pi}{3}, 0, 0, 0, 0, 0\right]^\mathrm{T}$')
    axs[0, 1].set_title(r'$\mathbf{m} = \left[\frac{\pi}{3}, 0, 0, \frac{\pi}{3}, 0, 0, 0, 0, 0\right]^\mathrm{T}$')
    axs[0, 2].set_title(r'$\mathbf{m} = \left[\frac{\pi}{3}, 0, 0, \frac{\pi}{3}, 0, 0, \frac{\pi}{3}, 0, 0\right]^\mathrm{T}$')
    axs[1, 0].set_title(r'$\mathbf{m} = \left[0, 0, 0, \frac{\pi}{3}, \frac{\pi}{3}, 0, 0, 0, 0\right]^\mathrm{T}$')
    axs[1, 1].set_title(r'$\mathbf{m} = \left[\frac{\pi}{3}, \frac{\pi}{3}, 0, \frac{\pi}{3}, \frac{\pi}{3}, 0, 0, 0, 0\right]^\mathrm{T}$')
    axs[1, 2].set_title(r'$\mathbf{m} = \left[\frac{\pi}{3}, \frac{\pi}{3}, 0, \frac{\pi}{3}, \frac{\pi}{3}, 0, \frac{\pi}{3}, \frac{\pi}{3}, 0\right]^\mathrm{T}$')
    axs[2, 0].set_title(r'$\mathbf{m} = \left[0, 0, 0, \frac{\pi}{3}, \frac{\pi}{3}, \frac{\pi}{3}, 0, 0, 0\right]^\mathrm{T}$')
    axs[2, 1].set_title(r'$\mathbf{m} = \left[\frac{\pi}{3}, \frac{\pi}{3}, \frac{\pi}{3}, \frac{\pi}{3}, \frac{\pi}{3}, \frac{\pi}{3}, 0, 0, 0\right]^\mathrm{T}$')
    axs[2, 2].set_title(r'$\mathbf{m} = \left[\frac{\pi}{3}, \frac{\pi}{3}, \frac{\pi}{3},\frac{\pi}{3}, \frac{\pi}{3}, \frac{\pi}{3},\frac{\pi}{3}, \frac{\pi}{3}, \frac{\pi}{3}\right]^\mathrm{T}$')
  

    # gs_kw1  = dict(width_ratios=[1,1,1], height_ratios=[1, 1, 1])

    # f1, axd1 = plt.subplot_mosaic([['1-1', '1-2', '1-3'], ['2-1', '2-2', '2-3'], ['3-1', '3-2', '3-3']], gridspec_kw=gs_kw1, figsize=(10, 12), layout="constrained")
    # Define 9 different shape states
    m_true1_1 = np.array([0, 0, 0, pi/3, 0, 0, 0, 0, 0])
    m_true1_2 = np.array([pi/3, 0, 0, pi/3, 0, 0, 0, 0, 0])
    m_true1_3 = np.array([pi/3, 0, 0, pi/3, 0, 0, pi/3, 0, 0])
    m_true2_1 = np.array([0, 0, 0, pi/3, pi/3, 0, 0, 0, 0])
    m_true2_2 = np.array([pi/3, pi/3, 0, pi/3, pi/3, 0, 0, 0, 0])
    m_true2_3 = np.array([pi/3, pi/3, 0, pi/3, pi/3, 0, pi/3, pi/3, 0])
    m_true3_1 = np.array([0, 0, 0, pi/3, pi/3, pi/3, 0, 0, 0])
    m_true3_2 = np.array([pi/3, pi/3, pi/3, pi/3, pi/3, pi/3, 0, 0, 0])
    m_true3_3 = np.array([pi/3, pi/3, pi/3, pi/3, pi/3, pi/3, pi/3, pi/3, pi/3])

    # plot the 9 different shape states

    # Plot the 3D curve with disks and pulling wires in subplot 1
    ax1 = fig.add_subplot(331, projection='3d')
    # ax1 = axd1['1-1']
    plot_3d_curve_with_disks_and_wires(ax1, m_true1_1, phi_func, k=20)

    # Plot the 3D curve with disks and pulling wires in subplot 2
    ax2 = fig.add_subplot(332, projection='3d')
    # ax2 = axd1['1-2']
    plot_3d_curve_with_disks_and_wires(ax2, m_true1_2, phi_func, k=20)

    # Plot the 3D curve with disks and pulling wires in subplot 3
    ax3 = fig.add_subplot(333, projection='3d')
    # ax3 = axd1['1-3']
    plot_3d_curve_with_disks_and_wires(ax3, m_true1_3, phi_func, k=20)

    # Plot the 3D curve with disks and pulling wires in subplot 4
    ax4 = fig.add_subplot(334, projection='3d')
    # ax4 = axd1['2-1']
    plot_3d_curve_with_disks_and_wires(ax4, m_true2_1, phi_func, k=20)

    # Plot the 3D curve with disks and pulling wires in subplot 5
    ax5 = fig.add_subplot(335, projection='3d')
    # ax5 = axd1['2-2']
    plot_3d_curve_with_disks_and_wires(ax5, m_true2_2, phi_func, k=20)

    # Plot the 3D curve with disks and pulling wires in subplot 6
    ax6 = fig.add_subplot(336, projection='3d')
    # ax6 = axd1['2-3']
    plot_3d_curve_with_disks_and_wires(ax6, m_true2_3, phi_func, k=20)

    # Plot the 3D curve with disks and pulling wires in subplot 7
    ax7 = fig.add_subplot(337, projection='3d')
    # ax7 = axd1['3-1']
    plot_3d_curve_with_disks_and_wires(ax7, m_true3_1, phi_func, k=20)

    # Plot the 3D curve with disks and pulling wires in subplot 8
    ax8 = fig.add_subplot(338, projection='3d')
    # ax8 = axd1['3-2']
    plot_3d_curve_with_disks_and_wires(ax8, m_true3_2, phi_func, k=20)

    # Plot the 3D curve with disks and pulling wires in subplot 9
    ax9 = fig.add_subplot(339, projection='3d')
    # ax9 = axd1['3-3']
    plot_3d_curve_with_disks_and_wires(ax9, m_true3_3, phi_func, k=20)
    ...
    # fig, axs = plt.subplots(3, 3, figsize=(10, 12), subplot_kw={'projection': '3d'}, constrained_layout=True)
    ...
    # # Define 9 different shape states
    # m_true_list = [
    #     np.array([0, 0, 0, pi/3, 0, 0, 0, 0, 0]),
    #     np.array([pi/3, 0, 0, pi/3, 0, 0, 0, 0, 0]),
    #     np.array([pi/3, 0, 0, pi/3, 0, 0, pi/3, 0, 0]),
    #     np.array([0, 0, 0, pi/3, pi/3, 0, 0, 0, 0]),
    #     np.array([pi/3, pi/3, 0, pi/3, pi/3, 0, 0, 0, 0]),
    #     np.array([pi/3, pi/3, 0, pi/3, pi/3, 0, pi/3, pi/3, 0]),
    #     np.array([0, 0, 0, pi/3, pi/3, pi/3, 0, 0, 0]),
    #     np.array([pi/3, pi/3, pi/3, pi/3, pi/3, pi/3, 0, 0, 0]),
    #     np.array([pi/3, pi/3, pi/3, pi/3, pi/3, pi/3, pi/3, pi/3, pi/3])
    # ]

    # # Plotting the 9 different shape states on the grid of subplots
    # for i, ax in enumerate(axs.flat):
    #     plot_3d_curve_with_disks_and_wires(ax, m_true_list[i], phi_func, k=20)

    # fig.tight_layout(rect=[0, 0.03, 1, 0.95])

    # plt.tight_layout()
    plt.show()
    # m_true = np.array([pi/3, pi/3, pi/3, pi/3,pi/3, pi/3, pi/3, pi/3, pi/3])
    # Plot the 3D curve with disks and pulling wires
